...
def download_missing_covers(missing_covers: list, output_dir: Path = ALBUM_COVERS_ORG) -> None:
    ...
    for item in missing_covers:
        tag_date = str(item.get("releaseYear") or item.get("date") or "0000")
        artist = item.get("display_artist") or item.get("artist") or item.get("ALBUM_ARTIST")
        album = item.get("title") or item.get("album") or item.get("ALBUM")
